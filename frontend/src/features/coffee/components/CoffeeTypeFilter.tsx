    ...
    {types.map((type) => (
      <button
        ...
        className={`flex-1 py-2 text-sm md:text-base font-medium tracking-wide transition-colors duration-200
          ${
            selected === type.value
              ? "bg-ui-white text-ui-typeText shadow-card hover:bg-mvst-orange hover:text-ui-white scale-100 hover:scale-110 transition-transform duration-200"
      ...
      >
        <Typography className="tracking-wide">{type.label}</Typography>
      </button>
    ))}
  </div>
);
